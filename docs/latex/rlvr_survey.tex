...
% Title and authors
\title{\LARGE\textbf{A Comprehensive Survey of Reinforcement Learning with Verifiable Rewards: \\[0.5em] From Language to Vision and Multimodal Agents}}

\author{
    % Author names to be filled in
    \textbf{Authors}\\[0.3em]
    \textit{Affiliation}\\[0.2em]
...
\begin{lstlisting}[caption={Core verification logic from \texttt{rlvr\_verifier.py}}, label=lst:verifier, language=Python, basicstyle=\ttfamily\tiny]
class RLVRVerifier:
    """Independent verifier for MiniGrid."""
    ...
    def verify_task_completion(self, s_state):
        """Apply formal verification rules."""
        if self.task_type == "reach_goal":
            goal_pos = s_state.get('goal_pos')
            agent_pos = s_state['agent_pos']
            ...
            if goal_pos and agent_pos == goal_pos:
                ...
                dist = abs(agent_pos[0] - goal_pos[0]) + \
                       abs(agent_pos[1] - goal_pos[1])
                return -0.001 * dist, {
                    'verified': False,
                    'explanation': f"Distance: {dist}"
                }
\end{lstlisting}
...
\begin{lstlisting}[caption={Environment integration from \texttt{rlvr\_env\_wrapper.py}}, label=lst:wrapper, language=Python, basicstyle=\ttfamily\tiny]
class RLVRMiniGridWrapper(gym.Wrapper):
    """Replaces env rewards with verified rewards."""
    
    def step(self, action):
        """Execute action, compute RLVR reward."""
        # Step 1: Execute in base environment
        obs, env_reward, done, trunc, info = \
            self.env.step(action)
        
        # Step 2: Extract symbolic state
        s_state = self.verifier.extract_symbolic_state(
            obs, self.env)
        
        if self.use_rlvr:
            # RLVR MODE - verified reward
            v_reward, v_info = \
                self.verifier.verify_task_completion(
                    s_state)
            reward = v_reward  # Use verified!
            
            # Add verification metadata
            info['rlvr_verification'] = v_info
            info['env_reward'] = env_reward
            info['reward_source'] = 'rlvr'
        else:
